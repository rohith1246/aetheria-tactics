/**
 * LaboratoryTreasureVaultRoomGenerator.js
 * Procedural Room Generator for Laboratory biome with TreasureVault dimensions.
 */

class LaboratoryTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'Laboratory';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.16999999999999998;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'laboratory_trap' });
          } else {
...
module.exports = { LaboratoryTreasureVaultRoomGenerator };
