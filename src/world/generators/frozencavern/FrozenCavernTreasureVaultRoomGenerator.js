/**
 * FrozenCavernTreasureVaultRoomGenerator.js
 * Procedural Room Generator for FrozenCavern biome with TreasureVault dimensions.
 */

class FrozenCavernTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'FrozenCavern';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.25;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'frozencavern_trap' });
          } else {
...
module.exports = { FrozenCavernTreasureVaultRoomGenerator };
