/**
 * CrystallineAbyssTreasureVaultRoomGenerator.js
 * Procedural Room Generator for CrystallineAbyss biome with TreasureVault dimensions.
 */

class CrystallineAbyssTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'CrystallineAbyss';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.41000000000000003;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'crystallineabyss_trap' });
          } else {
...
module.exports = { CrystallineAbyssTreasureVaultRoomGenerator };
