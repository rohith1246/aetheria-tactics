/**
 * ObsidianKeepTreasureVaultRoomGenerator.js
 * Procedural Room Generator for ObsidianKeep biome with TreasureVault dimensions.
 */

class ObsidianKeepTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'ObsidianKeep';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.43000000000000005;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'obsidiankeep_trap' });
          } else {
...
module.exports = { ObsidianKeepTreasureVaultRoomGenerator };
