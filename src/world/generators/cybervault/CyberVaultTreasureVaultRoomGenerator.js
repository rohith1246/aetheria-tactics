/**
 * CyberVaultTreasureVaultRoomGenerator.js
 * Procedural Room Generator for CyberVault biome with TreasureVault dimensions.
 */

class CyberVaultTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'CyberVault';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.19;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'cybervault_trap' });
          } else {
...
module.exports = { CyberVaultTreasureVaultRoomGenerator };
