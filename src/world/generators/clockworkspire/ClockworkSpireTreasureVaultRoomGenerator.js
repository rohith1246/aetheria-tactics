/**
 * ClockworkSpireTreasureVaultRoomGenerator.js
 * Procedural Room Generator for ClockworkSpire biome with TreasureVault dimensions.
 */

class ClockworkSpireTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'ClockworkSpire';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.37;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'clockworkspire_trap' });
          } else {
...
module.exports = { ClockworkSpireTreasureVaultRoomGenerator };
