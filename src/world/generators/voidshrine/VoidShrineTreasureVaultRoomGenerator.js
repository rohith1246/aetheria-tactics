/**
 * VoidShrineTreasureVaultRoomGenerator.js
 * Procedural Room Generator for VoidShrine biome with TreasureVault dimensions.
 */

class VoidShrineTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'VoidShrine';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.29000000000000004;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'voidshrine_trap' });
          } else {
...
module.exports = { VoidShrineTreasureVaultRoomGenerator };
