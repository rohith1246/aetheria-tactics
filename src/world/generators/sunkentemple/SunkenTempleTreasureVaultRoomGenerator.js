/**
 * SunkenTempleTreasureVaultRoomGenerator.js
 * Procedural Room Generator for SunkenTemple biome with TreasureVault dimensions.
 */

class SunkenTempleTreasureVaultRoomGenerator {
  constructor(seed = Date.now()) {
    this.seed = seed;
    this.theme = 'SunkenTemple';
    this.sizeCategory = 'TreasureVault';
    this.width = 26;
    this.height = 20;
    this.obstacleDensity = 0.35;
  }
      ...
      for (let x = 0; x < this.width; x++) {
        if (x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1) {
          ...
          if (rand < this.obstacleDensity) {
            ...
            row.push({ type: 'HAZARD', passable: true, hazardType: 'sunkentemple_trap' });
          } else {
...
module.exports = { SunkenTempleTreasureVaultRoomGenerator };
