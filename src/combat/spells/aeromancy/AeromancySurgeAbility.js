/**
 * AeromancySurgeAbility.js
 * School: aeromancy | Form: Surge
 * Tactical Combat Ability Engine Specification
 */

class AeromancySurgeAbility {
  constructor(config = {}) {
    this.id = 'ability_aeromancy_surge';
    this.name = 'AeromancySurge';
    this.school = 'aeromancy';
    this.archetype = 'Surge';
    this.tier = 2;
    this.manaCost = config.manaCost || 47;
    this.staminaCost = config.staminaCost || 15;
    this.baseCooldown = config.cooldown || 2;
    this.currentCooldown = 0;
    this.basePower = config.basePower || 118;
    this.castRange = config.castRange || 3;
    this.areaOfEffectRadius = config.radius || 2;
    this.castTimeTicks = config.castTime || 1;
    this.statusEffectId = 'status_aeromancy_surge';
...
module.exports = { AeromancySurgeAbility };
